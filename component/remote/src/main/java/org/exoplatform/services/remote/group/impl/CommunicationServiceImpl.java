...
public class CommunicationServiceImpl implements CommunicationService {
  ...
  synchronized public Object send(MemberInfo member, Message message) throws Exception {
    ...
    if(dest == null) throw new Exception("Cannot find the member: " + member.getIpAddress() );
  ...
  synchronized public void broadcast(Message message, boolean include) throws Exception {
    Vector members = null ;
    if(!include) members = getMembersExcludeMe() ;
  ...
  synchronized public List pingAll(String message) throws Exception  {
    ...
    for(int i = 0; i < res.size(); i++) {
      ...
      PingResult result = 
        new PingResult(Util.createMemberInfo(rsp.getSender()), (String)rsp.getValue()) ;
      results.add(result) ;
    }
    return results ;
  }
