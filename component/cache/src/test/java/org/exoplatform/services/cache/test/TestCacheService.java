...
public class TestCacheService extends BasicTestCase {
  ...
  public void testCacheService() throws Exception {
    ...
    assertTrue("expect found serializable key and it's value", sobj != null
        && sobj.equals("serializable object"));
    ...
    assertTrue("expect object has key is 'key1' is remove automatically",
               fifoCache.get("key1") == null);
    // -------remove a object in cache by key
    fifoCache.remove("key2");
    assertEquals("now, expect cache size is", 2, fifoCache.getCacheSize());
    assertEquals("now, expect number of object in cache is:", 2, fifoCache.getCachedObjects()
                                                                          .size());
    ...
    assertEquals("now, expect number of object in cache is:", 1, fifoCache.getCachedObjects()
                                                                          .size());
    ...
    assertEquals("now, expect number of object in cache is:", 0, fifoCache.getCachedObjects()
                                                                          .size());
