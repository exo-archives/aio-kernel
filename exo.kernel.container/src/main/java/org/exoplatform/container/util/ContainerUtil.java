  ...
  static public void addContainerLifecyclePlugin(ExoContainer container, ConfigurationManager conf) {
    ...
    while (i.hasNext()) {
      ...
      try {
        ...
        org.exoplatform.container.ContainerLifecyclePlugin instance = (org.exoplatform.container.ContainerLifecyclePlugin) classType.newInstance();
      ...
      try {
        ...
        if (key == null) {
          if (component.isMultiInstance()) {
            container.registerComponent(new ConstructorInjectionComponentAdapter(classType,
                                                                                 classType));
            System.out.println("===>>> Thread local component " + classType.getName()
                + " registered.");
          } else {
          ...
          try {
            ...
            if (component.isMultiInstance()) {
              container.registerComponent(new ConstructorInjectionComponentAdapter(keyType,
                                                                                   classType));
              System.out.println("===>>> Thread local component " + classType.getName()
                  + " registered.");
            } else {
